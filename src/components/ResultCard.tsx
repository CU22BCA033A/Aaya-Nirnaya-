...
function CardShell({
  ...
  return (
    <motion.div
      ...
      className={`group relative flex gap-4 overflow-hidden rounded-2xl border ${cls.border} bg-limewash-raised p-4 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_0_0_1px_var(--glow),0_16px_36px_-16px_var(--glow)] sm:p-5`}
    >
      <span
        aria-hidden="true"
        className="absolute inset-x-0 top-0 h-[3px] bg-gradient-to-r from-swarna/10 via-swarna to-swarna/10"
      />
      <span className="absolute -left-3 -top-3 flex h-7 w-7 items-center justify-center rounded-full border-2 border-swarna/70 bg-limewash-raised font-display text-xs font-semibold text-swarna-deep shadow-sm">
        {index}
      </span>
      {children}
    </motion.div>
